...
feedData(FD,Count,Len) ->
		Data = [random:uniform(2)+48 || _ <- lists:seq(1, Len)],
		writeData(FD,Data),
...
testRead(FileName) ->
		...
		List = getData(FD,Len,Len),
		file:close(FD),
		List.

getData(FD,Len,Count) ->
                {Acc} = readData(FD,Len),
		getData(FD,Len,Count-1,[Acc]).
		
getData(_FD,_Len,0,List) -> List;
getData(FD,Len,Count,List) ->
                {Acc} = readData(FD,Len),
		getData(FD,Len,Count-1,List++[Acc]).
